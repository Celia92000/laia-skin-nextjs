...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json(
        { error: 'Type de fichier non autorisé. Utilisez MP4, WebM, OGG ou MOV.' },
        { status: 400 }
      );
    }
    ...
    const uploadsDir = path.join(process.cwd(), 'public', 'uploads', 'videos', folder);
    try {
    ...
    const publicUrl = `/uploads/videos/${folder}/${fileName}`;
