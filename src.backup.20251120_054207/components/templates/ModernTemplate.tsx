/**
 * Template Modern - Basé sur le design actuel de LAIA SKIN INSTITUT
 * Style futuriste et dynamique avec animations et gradients
 * TOUTES LES DONNÉES SONT IDENTIQUES AU VRAI SITE, SEUL LE DESIGN PEUT VARIER
 */

import Link from 'next/link'
import { TemplateData } from '@/lib/template-data-loader'
import { Clock, ArrowRight, Sparkles, Star } from 'lucide-react'
import {
  getDisplayPrice,
  getForfaitDisplayPrice,
  hasPromotion,
...
export default function ModernTemplate({ data }: ModernTemplateProps) {
  const { organization, services, config, testimonials } = data
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto">
          <h1
            className={`text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-playfair mb-8 animate-fade-in-up leading-tight tracking-normal ${
              config.heroImage ? 'text-white' : 'text-[#2c3e50]'
            }`}
          >
            <span className="block font-normal">
              {config.heroTitle || 'Une peau respectée,'}
            </span>
            <span
              className={`block font-semibold mt-1 ${
                config.heroImage ? 'text-white/90' : 'text-[#d4b5a0]'
              }`}
            >
              {config.heroSubtitle || 'une beauté révélée'}
            </span>
          </h1>
          <p
            className={`font-inter text-base sm:text-lg md:text-xl mb-8 sm:mb-12 max-w-3xl mx-auto animate-fade-in-up animation-delay-200 tracking-normal ${
              config.heroImage ? 'text-white/90' : 'text-[#2c3e50]/60'
            }`}
          >
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up animation-delay-400">
            <Link
              href={`/${organization.slug}/reservation`}
              className="bg-gradient-to-r from-[#d4b5a0] to-[#c9a084] text-white px-6 sm:px-10 py-3 sm:py-4 rounded-full font-semibold text-base sm:text-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
            >
              Réserver un Soin
            </Link>
            <Link
              href={`/${organization.slug}/prestations`}
              className="bg-white text-[#2c3e50] px-6 sm:px-10 py-3 sm:py-4 rounded-full font-semibold text-base sm:text-lg shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
            >
              Découvrir nos Soins
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="font-inter text-base md:text-lg text-[#2c3e50]/60 max-w-2xl mx-auto tracking-normal">
              Découvrez notre gamme exclusive de soins innovants pour une peau
              éclatante et rajeunie
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {services.map((service) => (
              <Link
                key={service.id}
                href={`/${organization.slug}/services/${service.slug}`}
      ...
      <section className="py-28 sm:py-32 md:py-40 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-[#fdfbf7] via-white to-[#f8f6f0]">
          <div
            ...
            style={{
              backgroundImage:
                'radial-gradient(circle at 2px 2px, #d4b5a0 1px, transparent 1px)',
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-24">
            <div className="mb-2">
              <span className="font-inter text-xs md:text-sm text-[#d4b5a0] tracking-[0.3em] uppercase font-medium">
                Notre philosophie
              </span>
            </div>
            <h2 className="font-playfair text-4xl md:text-5xl lg:text-6xl text-[#2c3e50] mb-6">
              <span className="font-light">L'Excellence</span>
              <span className="block mt-2 font-normal italic text-[#d4b5a0]">
                à votre Service
              </span>
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-16 lg:gap-20">
            ...
            <div className="group">
              <div className="text-center transform transition-all duration-700 hover:-translate-y-3">
                ...
                <h3 className="font-playfair text-2xl md:text-3xl text-[#2c3e50] mb-5">
                  ...
                  <span className="block text-lg md:text-xl font-normal italic text-[#d4b5a0] mt-1">
                    Certifiée
                  </span>
                </h3>
                <p className="font-inter text-[#2c3e50]/60 leading-relaxed text-sm md:text-base px-6 font-light tracking-wide">
                  Plus d'une décennie dédiée à l'art de sublimer votre peau avec
                  des techniques d'exception
                </p>
              </div>
            </div>
            ...
            <div className="group">
              <div className="text-center transform transition-all duration-700 hover:-translate-y-3">
                ...
                <p className="font-inter text-[#2c3e50]/60 leading-relaxed text-sm md:text-base px-6 font-light tracking-wide">
                  Équipements de pointe minutieusement sélectionnés pour leur
            ...
            <div className="group">
              <div className="text-center transform transition-all duration-700 hover:-translate-y-3">
                ...
                <p className="font-inter text-[#2c3e50]/60 leading-relaxed text-sm md:text-base px-6 font-light tracking-wide">
                  Chaque protocole est méticuleusement conçu pour répondre à vos
                  besoins uniques
                </p>
              </div>
            </div>
          ...
          <div className="mt-32 text-center">
            <div className="max-w-4xl mx-auto relative">
              <div className="absolute -top-10 left-1/2 transform -translate-x-1/2">
                <svg
                  className="w-16 h-16 text-[#d4b5a0]/20"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div className="mt-10">
                <div className="inline-block">
                  <p className="font-inter text-sm text-[#d4b5a0] font-medium tracking-[0.3em] uppercase">
                    {config.founderName || 'Votre nom'}
                  </p>
                  <p className="font-inter text-xs text-[#2c3e50]/40 mt-2 tracking-wider">
                    {config.founderTitle ||
                      'Fondatrice & Experte en soins esthétiques'}
                  </p>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {testimonials.slice(0, 3).map((testimonial: any, index: number) => (
                <div
                  key={index}
                  className="bg-gradient-to-br from-[#fdfbf7] to-white rounded-2xl p-8 shadow-lg"
                >
                  <div className="flex mb-4">
                    {[...Array(testimonial.rating || 5)].map((_, i) => (
                      <svg
                        key={i}
                        className="w-5 h-5 text-[#d4b5a0] fill-current"
                        viewBox="0 0 20 20"
                      >
      ...
      <section className="py-24 bg-gradient-to-br from-[#d4b5a0] to-[#c9a084] text-white">
        <div className="max-w-4xl mx-auto text-center px-4">
          ...
          <p className="text-xl mb-12 opacity-95">
            Réservez dès maintenant votre soin personnalisé et découvrez la
            différence LAIA SKIN
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href={`/${organization.slug}/reservation`}
              className="bg-white text-[#2c3e50] px-10 py-4 rounded-full font-semibold text-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
            >
              Réserver un soin
            </Link>
            <Link
              href={`/${organization.slug}/contact`}
              className="bg-transparent border-2 border-white text-white px-10 py-4 rounded-full font-semibold text-lg hover:bg-white hover:text-[#2c3e50] transition-all duration-300"
            >
              Nous contacter
            </Link>
          </div>
