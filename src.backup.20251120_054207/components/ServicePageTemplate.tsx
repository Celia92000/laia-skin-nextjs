...
export default function ServicePageTemplate({ slug }: ServicePageTemplateProps) {
    ...
    return (
      <main className="pt-24 pb-20 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-accent/60">Chargement...</p>
        </div>
      </main>
    );
  }
  ...
  if (!service) {
    return (
      <main className="pt-24 pb-20 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-playfair text-accent mb-4">Service non trouvé</h1>
  ...
  return (
    <main className={`pt-24 pb-20 min-h-screen ${enrichment.isSignature ? 'bg-gradient-to-b from-[#fdfbf7] via-white to-[#f8f6f0]' : 'bg-gradient-to-b from-white to-gray-50/30'}`}>
      ...
      <section className="relative overflow-hidden">
        {enrichment.isSignature && (
          <div className="absolute inset-0">
            ...
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tl from-secondary/10 to-transparent rounded-full blur-3xl"></div>
          </div>
        )}
        {!enrichment.isSignature && (
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              <div>
                ...
                <h1 className={`${enrichment.isSignature ? 'text-6xl lg:text-7xl' : 'text-5xl lg:text-6xl'} font-playfair text-accent mb-6`}>
                  {enrichment.isSignature ? (
                    <>
                      <span className="bg-gradient-to-r from-[#d4b5a0] to-[#c9a084] bg-clip-text text-transparent">
                        {service.name.split(' ')[0]}
                      </span>
                      <span className="text-accent"> {service.name.split(' ').slice(1).join(' ')}</span>
                    </>
                  ) : (
                ...
                <p className="text-xl text-accent/70 leading-relaxed mb-4">
                  {service.description}
                </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-3 p-3 bg-white rounded-xl shadow-sm">
                  ...
                  <div>
                    <p className="text-sm text-accent/60">Durée</p>
                    <p className="font-semibold text-accent">{service.duration} min</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-white rounded-xl shadow-sm">
                  <Star className="w-8 h-8 text-amber-500" />
                  <div>
                    <p className="text-sm text-accent/60">Satisfaction</p>
                    <p className="font-semibold text-accent">{enrichment.stats[3].percentage}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="relative lg:scale-110 lg:ml-8">
              ...
              <div className="relative rounded-3xl overflow-hidden shadow-2xl" style={{ minHeight: slug === 'hydro-naissance' ? '500px' : 'auto' }}>
                ...
                <div className={`absolute bottom-6 right-6 ${enrichment.isSignature ? 'bg-gradient-to-br from-[#d4b5a0] to-[#c9a084]' : 'bg-white/95'} backdrop-blur-md rounded-2xl p-4 shadow-xl ${enrichment.isSignature ? 'px-5 py-4' : ''}`}>
                  ...
                  <p className={`text-sm ${enrichment.isSignature ? 'text-white/90' : 'text-accent/60'} mb-1`}>Tarif unique</p>
                  <div className="flex items-baseline gap-2">
                    ...
                    {isPromo && (
                      <>
                        <span className={`text-sm ${enrichment.isSignature ? 'text-white/60' : 'text-accent/60'} line-through`}>{service.price}€</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {enrichment.stats.map((stat: any, index: number) => (
              <div key={index} className="text-center">
                ...
                <p className={`text-sm ${enrichment.isSignature ? 'font-medium' : ''} text-accent/60`}>{stat.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="details-section" className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl lg:text-4xl font-playfair text-accent mb-4">
              Les bienfaits exceptionnels du {service.name}
            </h2>
            <p className="text-lg text-accent/60 max-w-3xl mx-auto">
              ...
              return (
                <div key={index} className="group">
                  <div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all hover:-translate-y-2 h-full">
                    ...
                    <p className="text-sm text-accent/60 mb-2">{benefit.desc}</p>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-semibold text-accent text-base">{step.title}</h4>
                          <span className="text-sm font-medium" style={{ color: enrichment.color }}>
                            {step.duration}
                          </span>
                        </div>
                        <p className="text-sm text-accent/70 leading-relaxed">{step.desc}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div>
              ...
              <div className="space-y-5">
                ...
                <div className="bg-white rounded-xl p-6 shadow-md">
                  <h4 className="font-semibold text-accent text-base mb-4">Contre-indications</h4>
                ...
                <div className="bg-white rounded-xl p-6 shadow-md">
                  <h4 className="font-semibold text-accent text-base mb-4">Avant votre soin</h4>
                  <ul className="space-y-3 text-sm text-accent/70">
                    <li className="flex items-start gap-2">
                ...
                <div className="bg-white rounded-xl p-6 shadow-md">
                  <h4 className="font-semibold text-accent text-base mb-4">Après votre soin</h4>
                  <ul className="space-y-3 text-sm text-accent/70">
                    <li className="flex items-start gap-2">
                      <CheckCircle className="w-4 h-4 text-green-500 mt-0.5" />
      ...
      <section className="py-20 px-4 bg-gradient-to-b from-white to-gray-50/30">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl lg:text-4xl font-playfair text-accent text-center mb-4">
            Nos formules sur mesure
          </h2>
          <p className="text-center text-lg text-accent/70 mb-12 max-w-3xl mx-auto">
            <span className="font-bold text-2xl" style={{ color: enrichment.color }}>1 séance à {displayPrice}€</span>
            {forfaitDisplayPrice && (
              <>
                <span className="mx-4 text-accent/40">|</span>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all transform hover:-translate-y-1 flex flex-col h-full">
              <div className="p-6" style={{
                ...
                <h3 className="text-xl font-semibold text-accent mb-2">Séance Découverte</h3>
                <div className="flex items-baseline gap-2">
                  ...
                  {isPromo && (
                    <>
                      <span className="text-sm text-accent/60 line-through">{service.price}€</span>
                ...
                <p className="text-sm text-accent/70 mt-1">Découvrez ce soin d'exception</p>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all transform hover:-translate-y-1 flex flex-col h-full">
              <div className="p-6 relative" style={{
                ...
                <div className="absolute top-2 right-2 bg-amber-400 text-accent text-xs font-bold px-2 py-1 rounded animate-pulse">
                  ⭐ BEST SELLER
                </div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="bg-amber-500 text-white text-xs font-bold px-2 py-1 rounded">FORFAIT 4 SÉANCES</span>
                </div>
                <h3 className="text-xl font-semibold text-accent mb-2">Cure Transformation</h3>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-bold" style={{ color: enrichment.color }}>
                    {forfaitDisplayPrice}€
                  </span>
                  {isForfaitPromo && service.forfaitPrice && (
                    <>
                      <span className="text-sm text-accent/60 line-through">{service.forfaitPrice}€</span>
                ...
                {forfaitSavings > 0 && (
                  <p className="text-sm text-accent/70 mt-1">Économisez <span className="font-bold" style={{ color: enrichment.color }}>{forfaitSavings}€</span></p>
                )}
              </div>
              <div className="p-6 flex flex-col flex-grow">
                <ul className="space-y-3 mb-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all transform hover:-translate-y-1 flex flex-col h-full">
              <div className="p-6" style={{
                ...
                <h3 className="text-xl font-semibold text-accent mb-2">Formule Liberté</h3>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-bold" style={{ color: enrichment.color }}>{monthlyPrice}€</span>
                  <span className="text-sm text-accent/60">/mois</span>
                </div>
                <p className="text-sm text-accent/70 mt-1">1 séance par mois • -20% sur le tarif normal</p>
              </div>
              <div className="p-6 flex flex-col flex-grow">
                <ul className="space-y-3 mb-6">
                  <li className="flex items-center gap-2 text-sm text-accent/70">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {enrichment.testimonials.map((testimonial: any, index: number) => (
              <div key={index} className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all">
                ...
                <p className="text-accent/70 text-sm italic">"{testimonial.comment}"</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl lg:text-4xl font-playfair text-accent text-center mb-12">
          ...
          <div className="space-y-4">
            {faqs.map((faq: any, index: number) => (
              <div key={index} className="bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-all">
                ...
                <p className="text-accent/70 pl-7">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-4 bg-gray-50/30">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-accent/60">
