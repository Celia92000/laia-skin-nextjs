...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      config: {
        siteName: organization.OrganizationConfig?.siteName,
        logoUrl: organization.OrganizationConfig?.logoUrl,
        primaryColor: organization.OrganizationConfig?.primaryColor || '#d4b5a0',
        secondaryColor: organization.OrganizationConfig?.secondaryColor || '#c9a084',
        accentColor: organization.OrganizationConfig?.accentColor || '#2c3e50',
      }
    });
  } catch (error) {
